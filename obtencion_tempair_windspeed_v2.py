...
def temp_speed(file_TS1, file_meteo):
    ...
    for a in file_TS1.index:
        ...
        for b in file_meteo.index:
            ...
            if (file_TS1['Fecha'][i][11:13] == file_meteo['hh:mm'][j][0:2]) and (file_TS1['Fecha'][i][14:16] == file_meteo['hh:mm'][j][3:5]):
                df['TEMP_AIR'][i] =  file_meteo['Temp.Ai2'][j]
                df['WIND_SPEED'][i] =  file_meteo['V.Vien.2'][j]
                w = w + 1
